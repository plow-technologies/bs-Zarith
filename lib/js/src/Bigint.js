...
function gcd(x, y) {
  ...
  while(true) {
    ...
    var a = _a;
    var c = erem(a, b);
    if (Caml_obj.caml_equal(c, zero$prime)) {
      return b;
    } else {
      _b = c;
      _a = b;
...
function sign(n) {
  if (Caml_obj.caml_equal(n, zero$prime)) {
    return 0;
  } else if (compare(n, zero$prime) < 0) {
    return -1;
  } else {
    return 1;
  }
